...
def export_to_onnx(model, model_name, output_dir, input_shape=(1, 3, 224, 224)):
    ...
    torch.onnx.export(
        ...
        output_path,
        export_params=True,
        opset_version=13,  # Updated to support aten::unflatten
...
def main():
    ...
    # Initialize models
    models_dict = initialize_models(len(class_names))
    ...
    for model_name, model in models_dict.items():
        ...
        if not model_path.exists():
            print(f"⚠️ Model weights not found at {model_path}, skipping {model_name}")
            continue
